...
class RedisClientSingleton {
  ...
  static getInstance(): RedisType | null {
    if (!config.REDIS_HOST) {
      RedisClientSingleton.logger.warn('Redis not configured, using memory cache');
      return null;
    }
    ...
    if (!RedisClientSingleton.instance) {
      try {
        const options: RedisOptions = {
          ...
          retryStrategy: (times: number) => {
            ...
            if (times > RedisClientSingleton.maxRetries) {
              RedisClientSingleton.logger.warn('⚠️  Redis unavailable - using memory cache');
        ...
        if (config.REDIS_PASSWORD !== undefined) {
          ...
          (options as RedisOptions & { password: string }).password = config.REDIS_PASSWORD;
        }
        ...
        RedisClientSingleton.instance!.on('error', (_error) => {
          if (!hasLoggedError) {
        ...
        RedisClientSingleton.logger.debug('Failed to create Redis client, using memory cache');
        return null;
      }
    }
